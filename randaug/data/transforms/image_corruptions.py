# Most of the code specified is taken from: https://github.com/bethgelab/imagecorruptions
#
...
from randaug.data.transforms.raindrops import DropModel
...
def gaussian_noise(x, severity=1, device=None):
...
def shot_noise(x, severity=1, device=None):
...
def impulse_noise(x, severity=1, device=None):
...
def speckle_noise(x, severity=1, device=None):
...
def gaussian_blur(x, severity=1, device=None):
...
def glass_blur(x, severity=1, device=None):
...
def defocus_blur(x, severity=1, device=None):
...
def motion_blur(x, severity=1, device=None):
...
def zoom_blur(x, severity=1, device=None):
...
def frost(x, severity=1, device=None):
    c = [(1, 0.4),
         (0.8, 0.6),
         (0.7, 0.7),
         (0.65, 0.7),
...
def rain(x, severity=1, device=None):
    ...
    rain_layer = np.random.normal(size=x.shape[:2], loc=c[0],
                                  scale=c[1])  # [:2] for monochrome


    rain_layer = clipped_zoom(rain_layer[..., np.newaxis], c[2])

    rain_layer[rain_layer < c[3]] = 0

    rain_layer = np.clip(rain_layer.squeeze(), 0, 1)

    rain_layer = _motion_blur(rain_layer, radius=c[4], sigma=c[5], angle=np.random.uniform(-135, -45))

    # The rain layer is rounded and cropped to the img dims
    rain_layer = np.round(rain_layer * 255).astype(np.uint8) / 255.
    rain_layer = rain_layer[..., np.newaxis]
    ...
    try:
        rain_layer = rain_layer + np.rot90(rain_layer, k=2)
        rain_layer = np.dstack((rain_layer, rain_layer, rain_layer)).astype(np.float32)
        x = cv2.addWeighted(x, 1.0, rain_layer, alpha, 0)
        return np.clip(x, 0, 1) * 255
    except ValueError:
        print('ValueError for rain, Exception handling')
        x[:rain_layer.shape[0], :rain_layer.shape[1]] += rain_layer + np.rot90(
            rain_layer, k=2)
        return np.clip(x, 0, 1) * 255
...
def snow(x, severity=1, device=None):
    ...
    zoom = c[2]
    threshold = c[3]
    radius, sigma = c[4], c[5]
    alpha = c[6]

    x = np.array(x, dtype=np.float32) / 255.
    snow_layer = np.random.normal(size=x.shape[:2], loc=loc,
                                  scale=scale)  # [:2] for monochrome

    snow_layer = clipped_zoom(snow_layer[..., np.newaxis], zoom)
    snow_layer[snow_layer < threshold] = 0
    #snow_layer[snow_layer > threshold[1]] = 0


    snow_layer = np.clip(snow_layer.squeeze(), 0, 1)

    snow_layer = _motion_blur(snow_layer, radius=radius, sigma=sigma, angle=np.random.uniform(-135, -45))
...
def spatter(x, severity=1, device=None):
...
def contrast(x, severity=1, device=None):
...
def brightness(x, severity=1, device=None):
...
def saturate(x, severity=1, device=None):
...
def jpeg_compression(x, severity=1, device=None):
...
def pixelate(x, severity=1, device=None):
...
def elastic_transform(image, severity=1, device=None):
...
def drops(image, severity=1, device=None):    
    ...
    drops_frequency = c[1]
    drops_shape = c[2]
    drops_sigma = torch.zeros(1).fill_(c[3]).to(device)

    dm = DropModel(imsize = (image.size[1], image.size[0]), size_threshold=1 / float(drops_size), frequency_threshold=drops_frequency, shape_threshold=drops_shape, device=device)

    im = T.ToTensor()(image) * 2 - 1
    im = im.unsqueeze(0).to(device)
    im_drops = dm.add_drops(im, sigma=drops_sigma)
    im_drops = ((im_drops[0].cpu() + 1.0) / 2.0) * 255
    im_drops = im_drops.permute(1, 2, 0)
...
corruption_tuple = (gaussian_noise, shot_noise, impulse_noise, defocus_blur,
                    glass_blur, motion_blur, zoom_blur, rain, snow, frost, fog,
...
def corrupt(image, severity=1, corruption_name=None, corruption_number=-1, device=None):
    ...
    print(image.dtype)
    # needs conversion to numpy array
...
def get_corruption_names(subset='common'):
    if subset == 'common':
        ...
        return [f.__name__ for f in corruption_tuple[7:8]]
    elif subset == 'digital':
